...
RSpec.describe Realms::Cards::Freighter do
  ...
  describe "#ally_ability" do
    ...
    it {
      ...
      expect(game.active_player.draw_pile.first).to eq(selected_card_1)
      expect(game.active_player.discard_pile).to_not include(selected_card_1)

      expect {
        game.acquire(selected_card_2)
      }.to change { game.active_player.discard_pile.length }.by(1)

      expect(game.active_player.discard_pile.first).to eq(selected_card_2)
      expect(game.active_player.draw_pile).to_not include(selected_card_2)
    }
  end
end
